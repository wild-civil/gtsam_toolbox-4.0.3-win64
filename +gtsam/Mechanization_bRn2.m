  ...
  methods
    function obj = Mechanization_bRn2(varargin)
      if nargin == 2 && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        my_ptr = varargin{2};
        gtsam_unstable_wrapper(444, my_ptr);
      elseif nargin == 0
        my_ptr = gtsam_unstable_wrapper(445);
      elseif nargin == 3 && isa(varargin{1},'gtsam.Rot3') && isa(varargin{2},'double') && isa(varargin{3},'double')
        my_ptr = gtsam_unstable_wrapper(446, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_unstable_wrapper(447, obj.ptr_gtsamMechanization_bRn2);
    end
    ...
    function varargout = bRn(this, varargin)
      ...
      varargout{1} = gtsam_unstable_wrapper(448, this, varargin{:});
    end

    function varargout = b_g(this, varargin)
      % B_G usage: b_g(double g_e) : returns Vector
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      varargout{1} = gtsam_unstable_wrapper(449, this, varargin{:});
    end
    ...
    function varargout = correct(this, varargin)
      ...
      if length(varargin) == 1 && isa(varargin{1},'double') && size(varargin{1},2)==1
        varargout{1} = gtsam_unstable_wrapper(450, this, varargin{:});
      else
    ...
    function varargout = integrate(this, varargin)
      ...
      if length(varargin) == 2 && isa(varargin{1},'double') && size(varargin{1},2)==1 && isa(varargin{2},'double')
        varargout{1} = gtsam_unstable_wrapper(451, this, varargin{:});
      else
    ...
    function varargout = x_a(this, varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      varargout{1} = gtsam_unstable_wrapper(452, this, varargin{:});
    end

    function varargout = x_g(this, varargin)
      % X_G usage: x_g() : returns Vector
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      varargout{1} = gtsam_unstable_wrapper(453, this, varargin{:});
    end
  ...
  methods(Static = true)
    function varargout = Initialize(varargin)
      ...
      if length(varargin) == 3 && isa(varargin{1},'double') && isa(varargin{2},'double') && isa(varargin{3},'double')
        varargout{1} = gtsam_unstable_wrapper(454, varargin{:});
      else
