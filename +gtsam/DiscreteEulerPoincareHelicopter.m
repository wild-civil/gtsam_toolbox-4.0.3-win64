...
classdef DiscreteEulerPoincareHelicopter < gtsam.NoiseModelFactor
  ...
  methods
    function obj = DiscreteEulerPoincareHelicopter(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_unstable_wrapper(295, varargin{2});
        end
        base_ptr = gtsam_unstable_wrapper(294, my_ptr);
      elseif nargin == 7 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'numeric') && isa(varargin{4},'double') && isa(varargin{5},'double') && isa(varargin{6},'double') && isa(varargin{7},'double')
        [ my_ptr, base_ptr ] = gtsam_unstable_wrapper(296, varargin{1}, varargin{2}, varargin{3}, varargin{4}, varargin{5}, varargin{6}, varargin{7});
      else
    ...
    function delete(obj)
      gtsam_unstable_wrapper(297, obj.ptr_gtsamDiscreteEulerPoincareHelicopter);
    end
