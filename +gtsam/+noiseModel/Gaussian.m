...
%Information(Matrix R) : returns gtsam::noiseModel::Gaussian
...
classdef Gaussian < gtsam.noiseModel.Base
  ...
  methods
    function obj = Gaussian(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(831, varargin{2});
        end
        base_ptr = gtsam_wrapper(830, my_ptr);
      else
    ...
    function delete(obj)
      gtsam_wrapper(832, obj.ptr_gtsamnoiseModelGaussian);
    end
    ...
    function varargout = print(this, varargin)
      ...
      if length(varargin) == 1 && isa(varargin{1},'char')
        gtsam_wrapper(838, this, varargin{:});
      else
  ...
  methods(Static = true)
    function varargout = Covariance(varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      if length(varargin) == 1 && isa(varargin{1},'double')
        varargout{1} = gtsam_wrapper(841, varargin{:});
      else
    ...
    function varargout = Information(varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      if length(varargin) == 1 && isa(varargin{1},'double')
        varargout{1} = gtsam_wrapper(842, varargin{:});
      else
        error('Arguments do not match any overload of function gtsam.noiseModel.Gaussian.Information');
      end
    end
    ...
    function varargout = SqrtInformation(varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      if length(varargin) == 1 && isa(varargin{1},'double')
        varargout{1} = gtsam_wrapper(843, varargin{:});
      else
