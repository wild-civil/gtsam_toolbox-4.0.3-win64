...
classdef StereoCamera < handle
  ...
  methods
    function obj = StereoCamera(varargin)
      if nargin == 2 && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        my_ptr = varargin{2};
        gtsam_wrapper(706, my_ptr);
      elseif nargin == 0
        my_ptr = gtsam_wrapper(707);
      elseif nargin == 2 && isa(varargin{1},'gtsam.Pose3') && isa(varargin{2},'gtsam.Cal3_S2Stereo')
        my_ptr = gtsam_wrapper(708, varargin{1}, varargin{2});
      else
    ...
    function delete(obj)
      gtsam_wrapper(709, obj.ptr_gtsamStereoCamera);
    end
  ...
  methods(Static = true)
    function varargout = Dim(varargin)
      % DIM usage: Dim() : returns size_t
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      varargout{1} = gtsam_wrapper(721, varargin{:});
    end
