...
classdef DGroundConstraint < gtsam.NonlinearFactor
  ...
  methods
    function obj = DGroundConstraint(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_unstable_wrapper(260, varargin{2});
        end
        base_ptr = gtsam_unstable_wrapper(259, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'double') && isa(varargin{3},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_unstable_wrapper(261, varargin{1}, varargin{2}, varargin{3});
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'double') && isa(varargin{3},'gtsam.noiseModel.Base')
    ...
    function delete(obj)
      gtsam_unstable_wrapper(263, obj.ptr_gtsamDGroundConstraint);
    end
